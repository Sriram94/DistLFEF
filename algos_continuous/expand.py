...
def train_expand(actor, critics, target_critics, actor_opt, critic_opts, buffer, expert_predictor):
    ...
    with torch.no_grad():
        expert_a = expert_predictor(s)
    
    expert_loss = F.mse_loss(curr_a, expert_a)
    actor_loss = (ALPHA * log_p - q_val).mean() + LAMBDA_EXP * expert_loss
...
def main():
    ...
    expert_predictor = lambda s: torch.zeros((s.size(0), ACTION_DIM)) 
    ...
    for ep in range(500):
        ...
        while not done:
            ...
            if len(buffer) > BATCH_SIZE:
                train_expand(actor, critics, target_critics, actor_opt, critic_opts, buffer, expert_predictor)
