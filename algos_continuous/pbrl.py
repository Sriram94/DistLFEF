...
def train_reward_model(reward_model, reward_opt, preference_buffer):
    seg1, seg2, labels = zip(*random.sample(preference_buffer, min(len(preference_buffer), BATCH_SIZE)))
    
    r1 = torch.stack([reward_model(s, a).sum() for s, a in seg1])
    r2 = torch.stack([reward_model(s, a).sum() for s, a in seg2])
    
    logits = torch.stack([r1, r2], dim=1)
    loss = F.cross_entropy(logits, torch.LongTensor(labels).to(logits.device))
    
    reward_opt.zero_grad()
    loss.backward()
    reward_opt.step()

def train_sac(actor, critics, target_critics, reward_model, actor_opt, critic_opts, buffer):
...
def main():
    ...
    preference_buffer = deque(maxlen=5000)
    trajectory_segment = []

    oracle = lambda s1, s2: 0 if np.sum(s1) > np.sum(s2) else 1 
    ...
    for ep in range(500):
        ...
        while not done:
            ...
            next_obs, env_reward, dones, _ = env.step({agent_id: action_np})
            buffer.append((state, action_np, env_reward[agent_id], next_obs[agent_id], dones[agent_id]))
            trajectory_segment.append((state, action_np, env_reward[agent_id]))
            
            state = next_obs[agent_id]
            done = dones[agent_id]

            if len(buffer) > BATCH_SIZE:
                train_sac(actor, critics, target_critics, reward_model, actor_opt, critic_opts, buffer)
            
            if len(trajectory_segment) >= SEGMENT_LENGTH:
                pass
