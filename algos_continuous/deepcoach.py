...
def train_deep_coach(actor, critics, coach, target_critics, actor_opt, critic_opts, buffer):
    batch = random.sample(buffer, BATCH_SIZE)
    s, a, r, s_, d = [torch.FloatTensor(np.array(x)) for x in zip(*batch)]
    
    with torch.no_grad():
        suggested_a = coach(s)
        coaching_reward = -F.mse_loss(a, suggested_a, reduction='none').mean(dim=1, keepdim=True)
        r_total = r + COACH_WEIGHT * coaching_reward
...
def main():
    ...
    coach = Coach(STATE_DIM, ACTION_DIM)
    ...
    for ep in range(500):
        ...
        while not done:
            ...
            if len(buffer) > BATCH_SIZE:
                train_deep_coach(actor, critics, coach, target_critics, actor_opt, critic_opts, buffer)
