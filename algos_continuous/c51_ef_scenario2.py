...
def get_epistemic_uncertainty_batch(ensemble, state, action, support):
    q_vals = torch.stack([torch.sum(net(state, action) * support, dim=-1) for net in ensemble])
    return torch.var(q_vals, dim=0)
...
def shape_reward(env_reward, h_val, sigma_eu):
    alpha_h = 1.0 / (1.0 + sigma_eu)
    return (1.0 - alpha_h) * env_reward + alpha_h * h_val
...
def train(actor, ensemble, h_head, target_ensemble, actor_opt, critic_opts, h_opt, buffer, support, predictor, budget):
    batch = random.sample(buffer, BATCH_SIZE)
    s, a, r, s_, d = [torch.FloatTensor(np.array(x)) for x in zip(*batch)]
    
    with torch.no_grad():
        sigma_eu = get_epistemic_uncertainty_batch(ensemble, s, a, support)
        noisy_r = torch.FloatTensor([get_noisy_reward(val.item()) for val in r]).to(s.device)
        norm_r = normalize_reward(noisy_r)
        h_scores = h_head(s)
        h_val = torch.mean(h_scores, dim=-1)
        final_rewards = shape_reward(norm_r, h_val, sigma_eu)

    with torch.no_grad():
        a_next, _ = actor(s_)
        for i in range(K_ENSEMBLE):
            next_probs = target_ensemble[i](s_, a_next)
            target_probs = projection_step(next_probs, final_rewards, d, support)
    ...
    q_vals_a = torch.stack([torch.sum(net(s, curr_a) * support, dim=-1) for net in ensemble])
    min_q = torch.min(q_vals_a, dim=0)[0].unsqueeze(-1)
    ...
    if budget['remaining'] > 0:
        ...
        with torch.no_grad():
            ...
            mask = (torch.rand(h_target.shape[0], h_target.shape[1]) > (1 - FEEDBACK_ACCURACY)).float()
...
def main():
    agent_id = "Scenario2_C51EF_Continuous"
    ...
    for ep in range(500):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                q_vals = torch.stack([torch.sum(net(state_t, actor_a) * support, dim=-1) for net in ensemble])
                eu = torch.var(q_vals, dim=0).mean().item()
                au = get_aleatoric_uncertainty(ensemble, state_t, actor_a, support)

            delta = 1.0 / (1.0 + au)
            epsilon = max(0.01, epsilon - delta)

            if random.random() < epsilon:
                if eu > EU_THRESHOLD and budget['remaining'] > 0:
                    action = h_head(state_t).detach().cpu().numpy()[0]
                else:
                    action = np.random.uniform(-1, 1, ACTION_DIM)
            else:
                action = actor_a.detach().cpu().numpy()[0]
            ...
            if len(buffer) > BATCH_SIZE:
                train(actor, ensemble, h_head, target_ensemble, actor_opt, critic_opts, h_opt, buffer, support, predictor, budget)
    env.close()
