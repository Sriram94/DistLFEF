...
class HIL:
    def __init__(
        ...
        self.epsilon = epsilon if not humaninput else 0
    ...
    def act(self, state):
        ...
        if np.random.random() < 1 - self.epsilon:
            preds = self.weight1*self.H.predict(state)+self.weight2*self.Q.predict(state) if self.humaninput else self.Q.predict(state)
    ...
    def _train_episode(self, episode_index, disp):
        ...
        with open(self.reward_log_path, 'a+', newline='') as write_obj:
            ...
            for ts in count():
                ...
                new_tuple = self.env.step(action)
                next_state = new_tuple[0]
