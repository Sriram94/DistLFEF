...
class C51_EF_Atari_S2(nn.Module):
    def __init__(self, n_actions):
        super(C51_EF_Atari_S2, self).__init__()
...
def train_c51_ef_s2(model, target_model, optimizer, buffer, device, noise_level=0.0):
    ...
    if noise_level > 0:
        noise = torch.randn_like(h_target) * noise_level
        h_target = h_target + noise
    ...
    # H-Head Training (Scenario 2: Feedback-Driven)
...
def main():
    ...
    model = C51_EF_Atari_S2(n_actions).to(device)
    target_model = C51_EF_Atari_S2(n_actions).to(device)
    ...
    for ep in range(1000):
        ...
        while not done:
            ...
            # Scenario 2 Gating: Rely on H-Head in novel/uncertain regions
            ...
            # Assume oracle provides a target action vector 'h_signal'
            h_signal = np.zeros(n_actions) 
            buffer.append((state, action, reward, next_obs, done, h_signal))
            state = next_obs

            if len(buffer) > 1000:
                train_c51_ef_s2(model, target_model, optimizer, buffer, device, noise_level=0.1)
